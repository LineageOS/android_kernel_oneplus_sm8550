 ...
 */
#define pr_fmt(fmt) "[DEEP_COVER_COPROC_SW]([%s][%d]): " fmt, __func__, __LINE__
				...
				u8 *ver_pubkey_x, u8 *ver_pubkey_y)
{
	...
	/* Compute the certificate */
	return deep_cover_verify_ecdsa_signature(message, msg_len, ver_pubkey_x, ver_pubkey_y, sig_r, sig_s);
}
